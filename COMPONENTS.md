...
|---|---|---|---|---|
| `compose/` | credentials from the environment, interpolated bare so no default can become a committed secret → seven named services with healthchecks, a one-shot provisioner gated by completion, named volumes, declared limits, and two profiles | The local spine: Spark master and workers, MinIO, Postgres, MLflow, Airflow, and the provisioner that creates the bucket MLflow's artifact root names (`docs/decisions/015`). Pinned tags, healthchecks, named volumes, declared limits, and the capped quickstart override. | M0 | built |
| `preflight/` | the environment, the runtime, and the fingerprint inside a kept volume → a pass, or a refusal naming the variable and what to do about it | The preconditions `up` depends on: a container runtime that answers, the seven credential variables present and non-placeholder, and whether a kept data volume was initialised with the credentials in the current `.env`. Not a documented step but a prerequisite, so it cannot be skipped (`docs/decisions/009`). | M0 | built |
...
| `images/` | `compose/` | Images the spine builds rather than pulls. One so far, and only because MLflow's published image ships neither of the drivers its own store flags accept (`docs/decisions/011`). Every `FROM` carries a tag and a digest and a contract test reads the Dockerfile to check it. Runs as a named non-root account with no login shell, carries OCI source labels, and every `pip install` pins with `==` (`docs/decisions/017`, `018`). | M0 | built |
| `supply/` | an SPDX document from `syft`, and a scan from `grype` → a sorted `name==version` inventory, and a gate that fails on an advisory ID absent from a committed baseline | The supply-chain reduction. Reproducible inventories rather than raw SBOMs, because an SBOM is not diffable and a diff is the whole point (`docs/decisions/019`). The gate keys on advisory *identity*, not severity, so a new finding fails and a known one does not (`docs/decisions/022`). Both scanner pins carry an expiry, because a scanner's usefulness is a database that must be fresh (`docs/decisions/020`). | M1 | built |
| `sbom/` | `supply/` | The committed evidence: one inventory and one advisory baseline per image. Committed rather than generated in CI, so a change in what an image contains shows up as a reviewable diff instead of as a number in a log nobody compares. | M1 | built |
| `charts/` | `.env`, and the image `images/` builds → three Deployments Available on a cluster, an Ingress that answers, and an HPA with a CPU number to act on | One versioned chart covering MLflow, Postgres and MinIO: Deployment, Service, Ingress, three probes, PVCs, an HPA, and no serving CRD (`docs/decisions/002`). Spark and Airflow are deliberately outside it, and the two stateful services are deliberately Deployments (`docs/decisions/024`). Carries no credential: `values.yaml` names a Secret and its keys, and the Secret is made from the same `.env` compose reads. `charts/kind-cluster.yaml` is the local cluster, not part of the release. | M2 | built; the cluster tier is green, and make kind-deploy is not |
